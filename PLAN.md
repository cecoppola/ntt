...
## 26. Phase 11 (revised for §25) — a multi-hour session with six agents (proposed 2026-09-20; supersedes §24)
...
Seams (agreed on day 0, additive): X needs subgroup creation from S's `mn.c` (`mn_group_at(level, size)` exists; S keeps its signature); L's `MN_GROUPS` schedule is read by S's communicator creation (L defines the parser in `rns_dist.c`, S calls it); P's init lines and V's deletions are in different regions of `ecalc.c`. Node protocol as §21. Timeline (hours): 0 launch; 0–4 agents (S's transport passes `t_comm` at hour ~2 and `t_dist` at ~3; L's grid at 9 processes by hour 2); 3–6 merges with the regression after each; 6–8 closing: five runs at 4 × 10¹⁰, 8 × 10¹⁰ and beyond, the §25 tables at g = 4 / 64 / 576 from X2 + M, paper v4.
