...
{
    uint64_t *tw = malloc(p->n / 2 * sizeof(uint64_t));
    if (!tw) return NULL;
    tw[0] = 1;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <n> <q> <omega> [iters]\n", argv[0]);
        fprintf(stderr, "  ML-KEM: %s 256 3329 17 100000\n", argv[0]);
        fprintf(stderr, "  ML-DSA: %s 256 8380417 1753 100000\n", argv[0]);
        return 1;
    }

    ntt_params_t p;
    p.n     = (uint64_t)atoll(argv[1]);
    p.q     = (uint64_t)atoll(argv[2]);
    p.omega = (uint64_t)atoll(argv[3]);
    uint64_t iters = (argc >= 5) ? (uint64_t)atoll(argv[4]) : 1;
