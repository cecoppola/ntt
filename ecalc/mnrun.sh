...
# allocation, several per node when there are fewer nodes (they share the node's four APUs); the
# node count used is the largest that divides <procs> (Slurm places tasks by blocks otherwise, and
# COMM_HOSTS must match the placement), listed explicitly.
