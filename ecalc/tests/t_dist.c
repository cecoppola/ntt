...
{
    int logn = logR + logC; size_t n = (size_t)1 << logn, rows = n / 4;
    ...
    size_t R = (size_t)1 << logR, C = (size_t)1 << logC, rr = R / 4;
    uint64_t *tmp = (uint64_t *)malloc(rows * 8);
    comm *cm[4]; dist_plan pl[4]; uint64_t *rx[4], *ry[4];
    for (int r = 0; r < 4; r++) {
        cm[r] = comm_sim4_create(r); dist_plan_create(&pl[r], cm[r], ctx, prime, logR, logC);
    ...
    for (int r = 0; r < 4; r++) dist_fwd_pre(&pl[r], rx[r], 0);
    for (int r = 0; r < 4; r++) dist_fwd_post(&pl[r], rx[r], 0);
    for (int r = 0; r < 4; r++) dist_fwd_pre(&pl[r], ry[r], 0);
    for (int r = 0; r < 4; r++) dist_fwd_post(&pl[r], ry[r], 0);
    for (int r = 0; r < 4; r++) dist_pw(&pl[r], rx[r], ry[r], 0);
    for (int r = 0; r < 4; r++) dist_inv_pre(&pl[r], rx[r], 0);
    for (int r = 0; r < 4; r++) dist_inv_post(&pl[r], rx[r], 0);
    ...
    for (int r = 0; r < 4; r++) {
        ...
        dist_plan_free(&pl[r]); comm_destroy(cm[r]); HIP_CHECK(hipFree(rx[r])); HIP_CHECK(hipFree(ry[r]));
    }
