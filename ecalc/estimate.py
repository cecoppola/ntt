...
LABELS = {
    ...
    "host": "modelled: 7 GB runtime + 4 GiB staging + the seed buffers at init + 6 GB per process of transport + the SHMEM pool (COMM_SHMEM_POOL_MB, 8 GiB)",
...
def estimate(g, D, tree="grid", groups=None, fabric=None, rule="model", transport="shmem", verbose=False):
    ...
    r = M.run(fab, D, g, rule, verbose=verbose, groups=groups, form=tree, transport=transport)
    m = r["mem"]
    out = dict(g=g, D=D, digits=r["digits"], wall_s=r["wall"], minutes=r["wall"] / 60,
               ...
               device_gb=m["device"], host_gb=m["host"], node_gb=m["node"], fits=m["node"] <= M.NODE_GB, fits_margin=m["node"] <= M.NODE_GB_MARGIN,
               tree=tree, schedule=r["schedule"], mem=m)
    return out

def fmt_b(b): return M.fmt_b(b)

def table(gs, Ds, tree, groups, fab, rule):
    print("%-4s %-8s %-10s | %7s %6s %6s %5s | %6s %6s %6s | %8s %8s %8s | %s" % ("g", "D/node", "digits", "wall s", "min", "expo s", "%", "dev GB", "hst GB", "node", "NIC/node", "per NIC", "global", "fits 502 / 480 GB"))
    rows = []
    for g in gs:
        for D in Ds:
            e = estimate(g, D, tree, groups, fab, rule)
            rows.append(e)
            print("%-4d %-8.1e %-10.3e | %7.1f %6.1f %6.1f %5.0f | %6.0f %6.0f %6.0f | %8s %8s %8s | %s / %s" % (
                g, D, e["digits"], e["wall_s"], e["minutes"], e["exposed_s"], e["exposed_pct"], e["device_gb"], e["host_gb"], e["node_gb"],
...
def main():
    ...
    print("ecalc estimate -- tree form %s, MN_GROUPS %s, fabric %.0f GB/s per APU, %.1f us per message, dragonfly group %d, %d layers, taper %.2f, part files %.1f GB/s per node"
          % (a.tree, a.groups or "(default)", a.bw, a.lat * 1e6, a.group, a.layers, a.taper, a.write_bw))
    if a.max:
        print("%-4s | %14s %10s %8s | %14s %10s %8s" % ("g", "max D @502 GB", "digits", "min", "max D @480 GB", "digits", "min"))
        for g in a.g:
            cells = []
            for budget in (M.NODE_GB, M.NODE_GB_MARGIN):
                D = M.max_digits(g, budget, a.tree, a.groups)
                e = estimate(g, D, a.tree, a.groups, fab, a.rule) if D else None
                cells.append("%14.2e %10.3e %8.1f" % (D, e["digits"], e["minutes"]) if e else "%14s %10s %8s" % ("-", "-", "-"))
            print("%-4d | %s | %s" % (g, cells[0], cells[1]))
        return
    if a.verbose:
        for g in a.g:
            for D in a.D: estimate(g, D, a.tree, a.groups, fab, a.rule, verbose=True)
    table(a.g, a.D, a.tree, a.groups, fab, a.rule)
    print()
    print("labels:")
