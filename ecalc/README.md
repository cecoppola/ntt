...
| `DM_TIGHT` | *Phase 14 (L1), APUMULT_STUDY E2*: the reciprocal's scratch reserved once at the last doubling's sizes instead of the final ones — r at ⌈k/2⌉ + 4 limbs, r2 at 2⌈k/2⌉ + 4, t1 at Q_t·r's size there — with r' copied into r's block (no block swap) at every doubling but the last; the device tier's top levels free each input pair as it is consumed; the block pool packs the large blocks (the top level's P, Q below the tail; t1 in the tail, r2 at the highest free end); `dm_layout` re-derived (the last doubling's set 5.2 n_Q + piece instead of 6.2, the top level 3.3 n_Q instead of 4.4, the division's own set as a term); bit-identical (0) |
| `DM_TAIL_DEAD` | *Phase 14 (L1), E5's layout half*: 1 = `dm_layout`'s v3 no longer reserves t1's hole beside the top level; after the top level the block pool's tail is re-laid over the level's dead inputs (`db_pool_retarget_tail`, a `dbig pool:` line under `DB_POOL_VERBOSE`); 2 = also v2 without P during the reciprocal — the layout the P spill needs, refused at init until the spill is wired (`BS_LAYOUT_ONLY` and `ECALC_INIT_ONLY=1` still print / map it) (0) |
| `MN_TREE_EARLY_FREE` | *Phase 14 (T1), APUMULT_STUDY E10a*: the multi-node tree's levels (`mn.c tree_level`, `tree_level_k`) free the dead P_i / P_run shares between a level's two products (P_n = P_i Q_run + P_run, then Q_n = Q_i Q_run), and `tree_need_dev` (mirrored in `mem_model.py`) counts max(2c + 2r + n, c + r + 2n) instead of 2c + 2r + 2n per device; `BS_LAYOUT_ONLY` prints `early_free`; `mem_model.py --e10a` gives the 576 ceilings; bit-identical (0) |
...
| `ECALC_ODIRECT` | *Phase 14 S1 (E3)*: 1 = the checkpoint files (level sets, tree sets, the top set in the background) and the output file are written with O_DIRECT through aligned buffers (`spill.c`'s `sp_file`; the last partial block padded, the file truncated to size), the small headers and the binary-base output with `fsync` + `posix_fadvise(DONTNEED)`: nothing written stays in the page cache (HBM on the APU); `mnaccept.sh` then compares with `dd iflag=direct` and no `.all` copy (0) |
| `ECALC_OUT_ODIRECT` | *Phase 14 S1*: with `ECALC_ODIRECT=1`, 0 keeps the output file buffered (1) |
| `ECALC_SPILL_DIR` | *Phase 14 S1 (E4)*: the directory of the spill primitive (`spill.h`: a dbig's limb range to one file per quarter, four APUs in parallel through a pinned double-buffered bounce with O_DIRECT, in the background; `SPILL_FREE` returns the blocks at completion, the restore reserves and reads back); unset = spilling disabled. The target: `/ssd0`; aac6: `/tmp` (unset) |
| `SPILL_CHUNK_MB` | *Phase 14 S1*: the spill's bounce buffers, two per APU on its NUMA node, pinned (256) |
| `SPILL_BUFFERED` | *Phase 14 S1*, *test*: the spill files without O_DIRECT (buffered, then `fsync` + DONTNEED) (0) |
| `ECALC_LIVE` | *Phase 14 S1 (E1)*: 1 = a `live:` line after every bs level, every tree level (size > 1), every Newton doubling and at the division's end: the block pool's live bytes per APU, the peak since the previous line, the pool's size and the peak's share of it, MemAvailable and Cached; also on with `ECALC_VERBOSE=2` (0) |
| `ECALC_MEM_SAMPLE` | *Phase 14 S1 (E12)*: seconds between memory samples (RSS, MemFree, MemAvailable, Cached, Dirty, Writeback from `/proc/meminfo`, the pool's live bytes per APU), a summary line at exit (minimum MemAvailable, maxima); unset = none |
| `ECALC_MEM_SAMPLE_FILE` | *Phase 14 S1*: the sampler's output file (`%d` = the node-process rank) instead of stderr (unset) |
| `ECALC_BUDGET_CHECK` | *Phase 14 C3 (E12)*: `1` = after init every rank models its peak (device in use or the model's planes + arena, whichever is larger, + the host terms or VmRSS) and the ranks decide together (an allgather, the same rule on the same numbers) whether a node is over `ECALC_NODE_GB` (default 480) or short of MemAvailable for what its ranks still take, or a rank over its share of the node; if so every rank stops before compute with the figures, rc 8 (`EC_RC_BUDGET`) (0) |
| `ECALC_BUDGET_RANK_GB` | *Phase 14 C3 (test hook)*: `<rank>:<GB>` sets that rank's share of the node budget (one rank over while its node is not) (unset) |
| `ECALC_FATAL_TEST` | *Phase 14 C3 (test)*: `1` = the four bs worker threads fail together right after the seeds (the in-phase guards' pattern): one message, rc 3 (0) |
| `ECALC_FATAL_WAIT` | *Phase 14 C3*: seconds the first failing thread may spend on its report (the memory accounting) before a watchdog `_exit`s (60) |
| `ECALC_FATAL_ABORT` | *Phase 14 C3*: `1` = a fatal error ends in `abort()` (a core file, rc 134) instead of `_exit(code)` (0). Codes (`fatal.h`): 3 an error inside the run (was `exit(1)` / `abort()`), 6 an allocation failed or a pool would grow inside a phase, 8 the budget check; the first failing thread prints one `ecalc: FATAL [rank r/n host h, APU d, thread t, after <phase>, rc c]: ...` line, the others block |
...
| `RNS_STRATEGY` | *Phase 13b (B)*: the single-node product's strategy (`dist_core`: top levels, reciprocal, division): `C` four-step; `B` prime-per-APU (planes that do not fit the pools from an extra per-APU buffer); `B4` B spread over four APUs at P = 3 (1.5 n points per APU); `auto` the B form where it fits the pools, else C (`auto` since Phase 13c) |
| `ECALC_PLANE_CAP` | *Phase 13b (P)*: the plane cap, `2^30`, `3*2^29`, `2^31`, `3*2^30` (or `30`, `3x29`, `31`, `3x30`) or `fit` (the largest whose node total fits `ECALC_NODE_GB`, default 480); sets `POOL_LOG`, `RNS_PLANES_3Q30`, `DIST_LOGN_TEST` and refuses a conflicting one; `off` = the pre-13c size rule (3·2³⁰ below 5 × 10¹⁰ digits, else 2³¹) (`2^31` since Phase 13c, unless one of those three is given) |
| `COMM_ALLTOALLV_DEPTH` | *Phase 13b (X)*: the uneven (`alltoallv`, general-map) layered exchange pipelined this many deep, 1 or 2; 2 hides the xGMI stage under the fabric (1.4 → 74 % on two real nodes), +1 quarter-plane of scratch per APU (2 since Phase 13c) |
| `NTT_B1R` | *Phase 13b (K)*: the b1 pass (stages 9..0) register-blocked, 8 points per thread and 3 LDS exchanges (`3`) or 16 (`4`, slower); 0 = the original pass; bit-identical (3 since Phase 13c) |
...
| `COMM_SHMEM_POOL_AUTO` | *Phase 14 (P2)*: size `COMM_SHMEM_POOL_MB` at init from the pool law (`binsplit_shmem_pool_need` = `mem_model.shmem_pool`: 4 APU threads × the largest staged exchange's send + receive + the control blocks + 256 MiB) when that exceeds the pool asked for; without it a pool below the need gets a warning naming the size and the run continues as before. A host heap (SOS without `COMM_SHMEM_DEVHEAP`, OSHMEM) also needs the SHMEM heap ≥ the pool + 512 MiB; `MN_PLAN_ONLY` prints both (`plan pool`) (0) |
| `COMM_SHMEM_VERBOSE` | *Phase 14 (P2)*: 1 = a line per new staging peak of the pool (the exchange, its send and receive) and every PE's pool summary at finalize (what occupied the pool at its peak); 2 = + the staged exchanges by call site (count, largest send / receive; object offsets for `llvm-symbolizer --obj=ecalc 0x200000+offset`). `ECALC_VERBOSE>=2` implies 1 (0) |
| `DIST_MN_SYM_SLABS` | *Phase 14 (P2), TASKS 2.4*: `mn_core`'s transform slabs (2 q) and the layered scratch (q) from the SHMEM symmetric pool, kept per APU (the inter-node transform stage unstaged). Measured: removes only the transform's staging (never the pool's peak: that is the result exchange) and adds 3 q × 8 B per APU of pool on top of plane pool 1 (+1536 MiB at 10⁹ on 2 nodes; 51.5 GB per node modelled at the 576 target) — not the design, kept off (0) |
...
| `MDB_SHIFT_CHUNK_MB` | *Phase 13a (M), TASKS 1.2*: the sharded division's `mdb_shift` exchanged in rounds of about this many MB per APU per slab (sb, rb) instead of the whole share / 4 (71 GB per node at 576 × 8 × 10¹⁰); bit-identical; 0 = one round (1024 since Phase 13c) |
| `MN_T_CHUNK_MB` | *Phase 13a (M), TASKS 1.3*: a piece product's window (the temporary T and the result slab rbO of `mn_core`, and `mdb_add_shifted`'s T) in rounds of this many MB per APU instead of the whole share of C (35 GB per node at 576 × 8 × 10¹⁰); the arena request (`rns_mul_dist_mn_scratch`) follows; bit-identical; 0 = one round (1024 since Phase 13c) |
