...
| `DM_TIGHT` | *Phase 14 (L1), APUMULT_STUDY E2*: the reciprocal's scratch reserved at its use — `r2` only when `d` is first written (2j + 4 limbs, freed whenever dead), `t1` per doubling at Q_t·r's size — and the device tier's top levels free each input pair as it is consumed; `dm_layout` re-derived (the last doubling's set 5.2 n_Q + piece instead of 6.2, the top level 3.3 n_Q instead of 4.4); bit-identical (0) |
