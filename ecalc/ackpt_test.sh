...
s1)   # single node 10^9, WP7's four runs
  D=1000000000; E="POOL_LOG=29 $CKE BS_CKPT_DIR=$CK/s1"
  run1 s1a $D $E;                       chk s1a $D; note "  sets after s1a: $(sets $CK/s1)"
  run1 s1b $D $E BS_RESTART=1;          chk s1b $D
  run1 s1c $D $E BS_CKPT_ABORT=8;       note "s1c: $(grep -E 'ABORT|checkpoint level' $OUT/s1c.log | tail -2 | tr '\n' ';')"; note "  sets after s1c: $(sets $CK/s1)"
...
s1v1) # a set written by main's ecalc (the v1 header and names), restarted by this branch
  D=1000000000; E="POOL_LOG=29 $CKE BS_CKPT_DIR=$CK/v1"
  runmain v1c $D $E BS_CKPT_ABORT=8;    note "v1c (main): $(grep -E 'ABORT' $OUT/v1c.log | tail -1)"; note "  sets: $(sets $CK/v1)"
...
s1dev) # single node 10^9 with device top levels in the leaf (BS_MDEV_LOGL=25): a set at a device-number level
  D=1000000000; E="POOL_LOG=29 BS_MDEV_LOGL=25 BS_CKPT_EVERY=1 BS_CKPT_MIN_LEVEL=2 BS_CKPT_DIR=$CK/s1dev"
  run1 s1deva $D $E;                    chk s1deva $D; L=$(mdev_level s1deva); note "  first mdev level $L; sets: $(sets $CK/s1dev)"
  run1 s1devc $D $E BS_CKPT_ABORT=$L;   note "s1devc: $(grep -E 'ABORT|checkpoint level' $OUT/s1devc.log | tail -2 | tr '\n' ';')"; note "  sets: $(sets $CK/s1dev)"
...
m8|m9) # sizes 2 and 4 on one node
  ...
  for p in 2 4; do
    T=$w.$p; E="POOL_LOG=$PL $CKE BS_CKPT_DIR=$CK/$T"
    runm ${T}a $p $D 900 $E;                              chk ${T}a $D; note "  sets after a: $(sets $CK/$T)"
    runm ${T}b $p $D 900 $E BS_RESTART=1;                 chk ${T}b $D   # from the top tree level's sets: straight to the gather
    runm ${T}c $p $D 900 $E BS_CKPT_ABORT=6;              note "${T}c: $(grep -c 'BS_CKPT_ABORT' $OUT/${T}c.log) nodes exited; sets: $(sets $CK/$T)"
    runm ${T}d $p $D 900 $E BS_RESTART=1;                 chk ${T}d $D   # from the leaf sets
    runm ${T}e $p $D 900 $E BS_CKPT_ABORT_TREE=1;         note "${T}e: $(grep -c 'BS_CKPT_ABORT_TREE' $OUT/${T}e.log) nodes exited; sets: $(sets $CK/$T)"
    runm ${T}f $p $D 900 $E BS_RESTART=1;                 chk ${T}f $D   # from tree level 1 (leaf sets skipped)
    runm ${T}g $p $D 60 $E BS_CKPT_ABORT=6 BS_CKPT_ABORT_NODE=1; note "${T}g: node 1 exited at leaf level 6, the rest killed by timeout; sets: $(sets $CK/$T)"
    ...
    runm ${T}t $p $D $KT $E;                              note "${T}t: killed after $KT s at: $(grep -E 'checkpoint|bs: level' $OUT/${T}t.log | tail -1); sets: $(sets $CK/$T)"
...
mdev) # 10^8 at sizes 2 and 4 with the leaf's device top levels (BS_MDEV_LOGL=21): sets at a device-number level of the leaf
  D=100000000
  for p in 2 4; do
    T=mdev.$p; E="POOL_LOG=27 BS_MDEV_LOGL=21 BS_CKPT_EVERY=1 BS_CKPT_MIN_LEVEL=2 BS_CKPT_DIR=$CK/$T"
    runm ${T}a $p $D 900 $E;                              chk ${T}a $D; L=$(mdev_level ${T}a); note "  first mdev level $L; sets: $(sets $CK/$T)"
    runm ${T}c $p $D 900 $E BS_CKPT_ABORT=$L;             note "${T}c: $(grep -c 'BS_CKPT_ABORT' $OUT/${T}c.log) nodes exited; sets: $(sets $CK/$T)"
