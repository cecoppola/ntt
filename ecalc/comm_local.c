...
{
    if (scnt[0] != rcnt[0]) { ec_fatal(EC_RC_FATAL, "comm_local: alltoallv count mismatch (%zu sent, %zu expected)\n", scnt[0], rcnt[0]); }
    c->priv = (void *)s;
...
{
    (void)c; if (scnt[0] != rcnt[0]) { ec_fatal(EC_RC_FATAL, "comm_local: alltoallv count mismatch\n"); }
