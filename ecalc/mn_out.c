...
static hipStream_t g_fs[DB_NQ]; static int g_fs_init;
static void dev_fetch(const dbig *x, size_t a, size_t cnt, uint64_t *host)
{
    ...
    if (!g_fs_init) { for (int d = 0; d < DB_NQ; d++) { HIP_CHECK(hipSetDevice(d)); HIP_CHECK(hipStreamCreateWithFlags(&g_fs[d], hipStreamNonBlocking)); } g_fs_init = 1; }
